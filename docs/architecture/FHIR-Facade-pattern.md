...
- Een Data Access Layer (DAL) specifiek voor het onderliggende niet-FHIR-native  systeem
...
# Toepasbaarheid
Een FHIR-facade wordt gebruikt om FHIR API's toe te voegen aan niet-FHIR-native  systemen in die gevallen waarin de vertaling van FHIR requests naar native queries/ API-calls, en weer terug van native response naar FHIR response, real-time plaats kan vinden. 
...
# Voor en nadelen
Een FHIR-facade is vaak sterker gekoppeld aan het onderliggende niet-FHIR-native  systeem dan een FHIR-store. Daardoor bevat een FHIR-facade minder herbruikbare componten dan een FHIR-store. Ook biedt de facade een directe attack factor op het onderliggende niet-FHIR-native  systeem juist omdat de facade zo sterk is gekoppeld aan het niet-FHIR-native systeem.
...
Voor data-intensieve requests kan een FHIR-facade veel stress veroorzaken op het onderliggende niet-FHIR-native  systeem, en daardoor primaire processen verstoren.
