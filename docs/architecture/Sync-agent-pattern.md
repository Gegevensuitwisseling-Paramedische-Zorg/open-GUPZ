...
# Bedoeling
Een FHIR-sync-agent repliceert data uit een niet-FHIR-native  systeem naar een FHIR compliant store, en (soms) vice versa. FHIR compliant clients kunnen zodoende data afkomstig uit het niet-FHIR-native  systeem benaderen voor lezen en (soms) schrijven. 

Door toepassing van een FHIR-sync-agent kunnen lagacy systemen FHIR compliant worden gemaakt. In het zogenaamde [FHIR besluit](https://open.overheid.nl/documenten/ronl-72d9d941c7ee7ae2c58c236290e152b22939448d/pdf) is door het ministerie van VWS besloten dat bij het beschikbaarstellen en uitwisselen van gezondheids(zorg)informatie gebruik dient te worden gemaakt van FHIR (versies STU3 of R4). Een FHIR-sync-agent voegt de daarvoor benodigde capabilities toe aan niet-FHIR-native  systemen.

# Alternatieven
FHIR-facades zijn een alternatief voor FHIR-stores en het sync-agent pattern. Een FHIR-facade vertaalt FHIR requests in realtime naar verzoeken op het onderliggende niet-FHIR-native  systeem, en de resultaten uit het niet-FHIR-native  systeem naar FHIR compliant responses
...
- Een sync-agent specifiek voor het onderliggende niet-FHIR-native  systeem. De syncy agent reageert (near) realtime of op basis van regels op wijzigingen in het onderliggende PARIS of op wijzigingen in de FHIR-store en zet een replicatie-mechanisme in werking
...
# Toepasbaarheid
Een FHIR-sync-agent wordt gebruikt om FHIR API's toe te voegen aan niet-FHIR-native  systemen in die gevallen waarin de vertaling van FHIR requests naar native queries/ API-calls, en weer terug van native response naar FHIR response, resource intensief en potentieel tijdrovend is.
